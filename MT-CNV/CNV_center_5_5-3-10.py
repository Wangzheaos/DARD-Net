...
def parameters():
    ...
    a = []
    with open('./results/CNV_center_5_5-3-10/record_CNV_center_5_5-3-10.txt') as f:
    ...
    for data_seed in range(0, 10):
        r = './results/CNV_center_5_5-3-10/' + str(a[data_seed]) + '/1260_' + str(data_seed) + '/transient/best.ckpt'
        # if not os.path.isfile(r+'.f'):
        #     r = './results/CNV_center_5_5-3-10/' + str(a[data_seed]) + '/1260_' + str(data_seed) + '/transient/checkpoint.185.ckpt'

        yield {
            **defaults,
            'title': '1260-label cnv',
            'n_labels': 1260,
            'data_seed': data_seed,
            'epochs': 200,
