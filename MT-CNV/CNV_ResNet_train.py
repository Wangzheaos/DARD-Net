...
def parameters():
    ...
    # a = []
    # with open('./results/CNV_train/record_CNV_train.txt') as f:
    ...
    for data_seed in range(0, 10):
        # r = './results/CNV_train/' + str(a[data_seed]) + '/3150_' + str(data_seed) + '/transient/best.ckpt'
        # if not os.path.isfile(r+'.f'):
        #     r = './results/CNV_train/' + str(a[data_seed]) + '/3150_' + str(data_seed) + '/transient/checkpoint.185.ckpt'
...
def run(title, base_batch_size, base_labeled_batch_size, base_lr, n_labels, data_seed, **kwargs):
    ...
    main_ResNet.args = parse_dict_args(**adapted_args, **kwargs)

    main_ResNet.main(context)
    # test(context)


def test(context):
    acc, auc, spe, sen, ema_acc, ema_auc, ema_spe, ema_sen = main_ResNet.main(context)
    acc_n.append(acc)
    auc_n.append(auc)
    spe_n.append(spe)
    sen_n.append(sen)
