...
def main(context):
    ...
    feature_encoder, relation_network = create_model()
...
def train(train_loader, center_loader, feature_encoder, relation_network, ema_feature_encoder, ema_relation_network,
    ...
    # for i, ((input, ema_input), target) in enumerate(train_loader):
        ...
        # calculate features
        center_features = feature_encoder(cent_input_var.cuda())  # 5x64*5*5
        batch_features = feature_encoder(input_var.cuda())  # 20x64*5*5

        ema_center_features = ema_feature_encoder(ema_cent_input_var.cuda())
        ema_batch_features = ema_feature_encoder(ema_input_var.cuda())
        ...
        class_loss = class_criterion(relations, target_var) / minibatch_size
        meters.update('class_loss', class_loss.item())

        ema_class_loss = class_criterion(ema_relations, target_var) / minibatch_size
