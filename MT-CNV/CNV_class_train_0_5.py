...
def parameters():
    ...
    a = []
    with open('./results/CNV_class_train_0_5/record_CNV_class_train_0_5.txt') as f:
    ...
    for data_seed in range(0, 10):
        r = './results/CNV_class_train_0_5/' + str(a[data_seed]) + '/3150_' + str(data_seed) + '/transient/best.ckpt'
        # if not os.path.isfile(r+'.f'):
        #     r = './results/CNV_class_train_0_5/' + str(a[data_seed]) + '/3150_' + str(data_seed) + '/transient/checkpoint.185.ckpt'
...
def test(context):
    acc, ema_acc = main_class.main(context)
    acc_n.append(acc)
...
def calculate():
    ...
    print("total: {} runs\n均值:{} | {}\n方差:{} | {}\n标准差:{} | {}".format(n, M, ema_M, S, ema_S, math.sqrt(S), math.sqrt(ema_S)))
