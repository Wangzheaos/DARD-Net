...
def main(context):
    ...
    if args.evaluate:
        ...
        acc, auc, spe, sen = validate(eval_loader, center_loader, feature_encoder, relation_network, validation_log, global_step,
                       args.start_epoch, accuracy_test)
        print("the primary model Acc:{}%".format(acc))
        LOG.info("Evaluating the EMA model:")
        ema_acc, ema_auc, ema_spe, ema_sen = validate(eval_loader, center_loader, ema_feature_encoder, ema_relation_network, ema_validation_log,
                           global_step, args.start_epoch, accuracy_test)
        print("the EMA model Acc:{}%".format(ema_acc))
        return acc, auc, spe, sen, ema_acc, ema_auc, ema_spe, ema_sen
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.evaluation_epochs and (epoch + 1) % args.evaluation_epochs == 0:
            start_time = time.time()
            LOG.info("Evaluating the primary model:")
            prec1, _, _, _ = validate(eval_loader, center_loader, feature_encoder, relation_network, validation_log, global_step,
                             epoch + 1, accuracy)
            LOG.info("Evaluating the EMA model:")
            ema_prec1, _, _, _ = validate(eval_loader, center_loader, ema_feature_encoder, ema_relation_network,
                                 ema_validation_log, global_step, epoch + 1, accuracy)
...
def train(train_loader, center_loader, feature_encoder, relation_network, ema_feature_encoder, ema_relation_network,
    ...
    for i, ((input, ema_input), target) in enumerate(train_loader):
        ...
        with torch.no_grad():
            ...
            cent_var = torch.autograd.Variable(cent_input)
        ...
        # calculate features
        # centers, _ = feature_encoder(cent_input.cuda())  # 5x64*5*5

        if epoch == 0 and i == 0:
            centersn, _ = feature_encoder(cent_var)
            feature_encoder.module.centers = centersn.detach()
            centersn, _ = ema_feature_encoder(cent_var)
            ema_feature_encoder.module.centers = centersn.detach()

        center_features = feature_encoder.module.centers
        batch_features, batch_features2 = feature_encoder(input_var.cuda())  # 20x64*5*5

        ema_center_features = ema_feature_encoder.module.centers
        # ema_center_features = ema_feature_encoder(ema_cent_input_var.cuda())
        ...
        relation_pairs = torch.cat((center_features_ext, batch_features_ext), 2).view(-1, args.feature_dim * 2, 18,
                                                                                      18)  # 拼接按第二维
        relations_t = relation_network(relation_pairs)
        relations = relations_t.view(-1, class_num * 1)
        relations2 = xitocu(relations, class_num)
        ...
        ema_relation_pairs = torch.cat((ema_center_features_ext, ema_batch_features_ext), 2).view(-1,
                                                                                                  args.feature_dim * 2,
                                                                                                  18, 18)
        ema_relations_t = ema_relation_network(ema_relation_pairs)
        ema_relations = ema_relations_t.view(-1, class_num * 1)
        ema_relations2 = xitocu(ema_relations, class_num)

        class_loss = class_criterion(relations2, target_var2) / minibatch_size
        meters.update('class_loss', class_loss.item())

        ema_class_loss = class_criterion(ema_relations2, target_var2) / minibatch_size
        ...
        center_loss = args.lamda * losses.compute_center_loss(batch_features[unlabeled_minibatch_size:],
                                                              center_features, target_var[unlabeled_minibatch_size:])
        # ema_center_loss = args.lamda * losses.compute_center_loss(ema_batch_features2[unlabeled_minibatch_size:],
        #                                                           ema_center_features,
        #                                                           target_var[unlabeled_minibatch_size:])
        meters.update('center_loss', center_loss.item())
        # meters.update('ema_center_loss', ema_center_loss.item())

        loss = class_loss + consistency_loss_1 + consistency_loss_2 + center_loss
        ...
        # if epoch == 0 & i == 0:
        center_deltas = losses.get_center_delta(batch_features[unlabeled_minibatch_size:].data, center_features,
                                                target_var[unlabeled_minibatch_size:], args.alpha)
        feature_encoder.module.centers = center_features - center_deltas
        ema_center_deltas = losses.get_center_delta(ema_batch_features[unlabeled_minibatch_size:].data,
                                                    ema_center_features, target_var[unlabeled_minibatch_size:],
                                                    args.alpha)
        ema_feature_encoder.module.centers = ema_center_features - ema_center_deltas
        ...
        if i % args.print_freq == 0:
            LOG.info(
                ...
                'Cens {meters[center_loss]:.4f}\t'
                # 'ema_Cens {meters[ema_center_loss]:.4f}\t'
                'Loss {meters[loss]:.4f}\t'
...
def validate(eval_loader, center_loader, feature_encoder, relation_network, log, global_step, epoch, accuracy_f):
    ...
    for i, (input, target) in enumerate(eval_loader):
        ...
        # calculate features
        # center_features, _ = feature_encoder(cent_input_var)  # 5x64*5*5

        center_features = feature_encoder.module.centers
        batch_features, _ = feature_encoder(input_var.cuda())  # 20x64*5*5
        ...
        relation_pairs = torch.cat((center_features_ext, batch_features_ext), 2).view(-1, args.feature_dim * 2, 18, 18)
        relations_t = relation_network(relation_pairs)
        relations = relations_t.view(-1, class_num * 1)
        ...
        prec1 = accuracy_f(relations.data, target_var.data, topk=1)
        meters.update('class_loss', class_loss.item(), labeled_minibatch_size)

        test = False
        if type(prec1) is tuple:
            prec1, auc, spe, sen = prec1
            meters.update('auc', auc)
            meters.update('spe', spe)
            meters.update('sen', sen)
            test = True
        ...
        if i % args.print_freq == 0:
            LOG.info(
                ...
                'Class {meters[class_loss]:.4f}\t'
                'Prec@1 {meters[top1]:.3f}'.format(i, len(eval_loader), meters=meters))
    ...
    if test:
        return meters['top1'].avg, meters['auc'].avg, meters['spe'].avg, meters['sen'].avg
    else:
        return meters['top1'].avg, meters['top1'].avg, meters['top1'].avg, meters['top1'].avg


def xitocu(relations, class_num):
    relations2 = torch.zeros([relations.size()[0], 3]).cuda()
    for i in range(0, int(class_num / 3)):
        relations2[:, 0] = relations2[:, 0] + relations[:, i]
    for i in range(int(class_num / 3), int(class_num / 3) * 2):
        relations2[:, 1] = relations2[:, 1] + relations[:, i]
    for i in range(int(class_num / 3) * 2, class_num):
        relations2[:, 2] = relations2[:, 2] + relations[:, i]
    return relations2
