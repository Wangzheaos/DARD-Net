...
def parameters():
    ...
    a = []
    with open('./results/CNV_center_5_0-3/record_CNV_center_5_0-3.txt') as f:
    ...
    for data_seed in range(0, 10):
        r = './results/CNV_center_5_0-3/' + str(a[data_seed]) + '/3150_' + str(data_seed) + '/transient/best.ckpt'
        # if not os.path.isfile(r+'.f'):
        #     r = './results/CNV_center_5_0-3/' + str(a[data_seed]) + '/3150_' + str(data_seed) + '/transient/checkpoint.185.ckpt'
...
def test(context):
    acc, ema_acc = main_class_center.main(context)
    acc_n.append(acc)
...
def calculate():
    ...
    print("total: {} runs\n均值:{:.2f} | {:.2f}\n方差:{:.2f} | {:.2f}\n标准差:{:.2f} | {:.2f}".format(n, M, ema_M, S, ema_S, math.sqrt(S), math.sqrt(ema_S)))
