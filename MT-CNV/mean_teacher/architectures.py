...
@export
def relationnet(feature_dim, relation_dim):
    model = RelationNetwork(feature_dim, relation_dim)
    return model
...
class CNNEncoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.register_buffer('centers', (
                torch.rand(6, 64, 18, 18).to("cuda") - 0.5) * 2)
        # 三分类特征
    ...
    def forward(self, x):
        ...
        # out2 = self.fc1(out2)
        # feature_normed = out2.div(
        #     torch.norm(out2, p=2, dim=1, keepdim=True).expand_as(out2))
...
class ResNet32x32(nn.Module):
    def __init__(self, block, layers, channels, groups=1, num_classes=3, downsample='basic'):
        super().__init__()
        assert len(layers) == 3
