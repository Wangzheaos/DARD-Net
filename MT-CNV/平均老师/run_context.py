...
class TrainLog:
    ...
    def __init__(self, directory, name):
        self.log_file_path = "{}/{}.html".format(directory, name)
    ...
    def save(self):
        ...
        self.df = df
        df.to_html(self.log_file_path)

    def read(self):
        if not self.df:
            return
        axes_subplot = self.df.plot()
        # print(type(axes_subplot)) #<class 'matplotlib.axes._subplots.AxesSubplot'>
        plt.xlabel("epoch")
        plt.ylabel("all")
        plt.legend(loc="best")
        plt.grid(True)
        img_name = os.path.join(self.directory, self.name + '.png')
        ...
        with open(self.log_file_path, 'w') as f:
            f.write(HEADER)
            f.write(self.df.to_html(classes='df'))
            f.write(FOOTER)
